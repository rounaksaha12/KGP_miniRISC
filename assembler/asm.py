    ...
    with open(source_file, 'r') as f:
        for lno, line in enumerate(f.readlines()):
            ...
            if line == '' or line[0] == '#':  # Skip empty lines and comments
                continue
            elif LABEL_RE.match(line):  # check if line is a label
                ...
                args = re.split(r'[\s,\(\)]+', line)    # Split the line into arguments
                ...
                if op in RFORMATS:  # r-format instructions
                    ...
                    if label in LTABLE:
                        # pc + 1 + offset where addr is offset
                        # compute addr
                        addr = LTABLE[label] - PC - 1
                        # get 2's complement of addr (21 bit)
                        if addr < 0:
                             addr = (1 << 21) + addr
